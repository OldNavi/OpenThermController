...
#define DT CURVE_KP + sizeof(int32_t)
...
static struct MAIN_VARIABLES
{
  ...
  VARIABLE<float> control_set = VARIABLE<float>(23.0);
...
public:
  ...
  void write()
  {
    ...
    Serial.println("Дельта времени = " + String(dT.value));
    commit();
  }
  ...
  void read()
  {
    ...
    dT.value = eprom.get(DT,dT.value);
